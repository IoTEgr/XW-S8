...
static u8 free_flag =2;	//usb reuse buff flag  .0:free ,1:busy
...
//	dispLayerUninit(DISP_LAYER_VIDEO);
	...
	else
	{
		...
		free_flag = 1;
		#endif
		if(/*SysCtrl.sdcard == SDC_STAT_NORMAL&& */arg==USB_DEV_MASS)
			mode = 0;	// 0 : card reader  , 1 : PC cam   , 2 card reader + PCcam
		else
			mode = 1;	// 0 : card reader  , 1 : PC cam   , 2 card reader + PCcam
		//set lcd video layer color
...
static int usbDeviceExit(winHandle handle,uint32 parameNum,uint32* parame)
{
	uint32 keyState=KEY_STATE_INVALID;
	if(parameNum==1)
		keyState=parame[0];
...
msgDealInfor usbDeviceMsgDeal[]=
{
	...
	{KEY_EVENT_RETURN,usbDeviceExit},
