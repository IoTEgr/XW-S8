...
#include "../inc/frame.h"
#include "../inc/timer.h"
#include "../inc/layer12.h"
#include "../inc/layer3.h"
...
int mad_header_decode(struct mad_header *header, struct mad_stream *stream)
{
  ...
  if (stream->skiplen) {
    ...
    if (end - ptr < stream->skiplen) {
      ...
      stream->next_frame = (unsigned char *)end;
 ...
 sync:
  ...
  if (stream->sync) {
    if (end - ptr < MAD_BUFFER_GUARD) {
      stream->next_frame = (unsigned char *)ptr;
      ...
      stream->this_frame = (unsigned char *)ptr;
      stream->next_frame = (unsigned char *)ptr + 1;
	...
	stream->next_frame = (unsigned char *)end - MAD_BUFFER_GUARD;
  ...
  /* begin processing */
  stream->this_frame = (unsigned char *)ptr;
  stream->next_frame = (unsigned char *)ptr + 1;  /* possibly bogus sync word */
